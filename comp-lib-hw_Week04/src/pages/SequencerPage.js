  ...
  return (
    <div className="flex flex-col gap-4 items-center  bg-black  p-10  ">
      <div className="flex items-center gap-3">
        {!isPlaying ? ( 
          <button onClick={handlePlay} className=" bg-[#e8ff00] px-3 py-1 roundedtext-white">
            Play
          </button>
        ) : (
          <button onClick={handleStop} className=" border-[#e8ff00] px-3 py-1 rounded bg-gray-200">
            Stop
          </button>
        )}
        <input type="range" min={60} max={200} value={bpm} onChange={(e) => setBpm(parseInt(e.target.value, 10))} />
      </div>
